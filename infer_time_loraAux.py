...
#这个版本是推理权重最好的qwen3vl_time_lora
#仅仅用了68&75的数据进行RAG
#仅仅针对于CholecTrack20进行RAG

#2改了一个fps细节
#3精简了一下prompt
#4根据不同领域调整了一下调用的模型，pet领域使用原始模型，其他领域使用lora模型；现在pet领域使用lora8_512，医学用原始，其他领域使用lora模型
...
def parse_args() -> argparse.Namespace:
    ...
    default_lora_adapter1="/root/data/wy/test/now/vqa/submit/lora_output/qwen3vl_time_lora"
    default_lora_adapter2="/root/data/wy/test/now/vqa/submit/lora_output/qwen3vl_time_lora_8f512_ep2_lr5e5"
    ...
    default_rag_root = project_root / "dataset" / "cholecT50_2"
    default_output = Path(__file__).resolve().parent / "qwen3vl_fps2_rag_lora512_adapted_interated2.json"
    ...
    parser.add_argument("--lora-adapter1", type=str, default=str(default_lora_adapter1), help="LoRA adapter directory")
    parser.add_argument("--lora-adapter2", type=str, default=str(default_lora_adapter2), help="LoRA adapter directory")
...
def main() -> None:
    ...
    # model = PeftModel.from_pretrained(base_model, args.lora_adapter1)
    
    model = PeftModel.from_pretrained(base_model, args.lora_adapter1, adapter_name="general_vqa")
    model.load_adapter(args.lora_adapter2, adapter_name="egopet_expert")
    ...
    for sample in tqdm(samples, desc="Infer"):
        ...
        if  dataset_name == "CholecTrack20" or dataset_name == "EgoSurgery":
            model.disable_adapter()
        elif dataset_name == "EgoPet" :
            model.set_adapter("egopet_expert")
        else:
            model.set_adapter("general_vqa")
        ...
        if sample['dataset']=='CholecTrack20' and ("VID25" in sample["video_path"][0] or "VID111" in sample["video_path"][0] ):
            fps=1.0
            interval=1.0
            print("CholecTrack20视频VID25和VID111片段均以 1 FPS 的速率提供")
        elif sample['dataset']=='EgoSurgery':
            fps=1.0
            interval=1.0
            print("EgoSurgery片段均以 1 FPS 的速率提供")
        else:
            fps=0.5
            interval=2.0
        # fps = 0.5
        ...
        content = []
        rag_images = []
        if (dataset_name == "CholecTrack20")and rag_index.get("records"):
